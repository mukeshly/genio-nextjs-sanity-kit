...
async function findExistingPage(env, documentId, slug) {
    const client = createSanityWriteClient(env);
    if (documentId) {
        return client.fetch(`*[_type == "page" && _id == $documentId][0]{_id, publishedAt, oldSlugs, "slug": slug.current}`, { documentId });
    }
    return client.fetch(`*[_type == "page" && slug.current == $slug][0]{_id, publishedAt, oldSlugs, "slug": slug.current}`, { slug });
}
...
export async function publishPage({ env, input, }) {
    return doPublishPage(env, validatePublishPageInput(input));
}
