...
import type { SitePageDocument } from "../types/blog.js";
...
export async function getAllSitePages(
  client: SanityClient,
  options?: { revalidate?: number; reservedRootSlugs?: string[] },
) {
  const reserved = createReservedRootSlugs(options?.reservedRootSlugs);
  const pages = await client.fetch<SitePageDocument[]>(
    SITE_PAGES_QUERY,
    {},
...
export async function getSitePageBySlug(
  ...
  const page = await client.fetch<SitePageDocument | null>(
    SITE_PAGE_BY_SLUG_QUERY,
    { slug },
    { next: { revalidate: options?.revalidate ?? 60 } },
  );
...
export async function getSitePageByOldSlug(
  client: SanityClient,
  slug: string,
  options?: { revalidate?: number; reservedRootSlugs?: string[] },
) {
